...
const AppUsageDetails = ({
  ...
  const [sortBy, setSortBy] = useState('time') // 'time', 'name', 'category'
      ...
      {isExpanded && (
        <CardContent className="p-0">
          <div className="bg-slate-800/30 rounded-b-lg overflow-hidden">
            {/* Header */}
            <div className="grid grid-cols-10 text-xs text-slate-400 p-3 border-b border-slate-700/50 bg-slate-800/50">
              <button
                onClick={() => handleSort('name')}
                className="col-span-5 flex items-center gap-1 hover:text-cyan-400 transition-colors text-left"
              >
                Application
                {getSortIcon('name')}
              </button>
              ...
              <div className="col-span-1">Productivity</div>
            </div>
            ...
            <div className="divide-y divide-slate-700/30 max-h-80 overflow-y-auto">
              {sortedApps.map((app, index) => (
                <div
                  key={`${app.name}-${index}`}
                  className="grid grid-cols-10 py-3 px-3 text-sm hover:bg-slate-800/50 transition-colors"
                ...
                >
                  ...
                  <div className="col-span-2 text-slate-400">{app.category || 'Unknown'}</div>
